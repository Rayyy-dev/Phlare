      ...
      <div className="space-y-5">
        ...
        {r.failed.length > 0 ? (
          <div className="card p-0">
            <p className="border-b border-ink-200 px-4 py-3 text-sm font-semibold text-red-700">
              ...
              <tbody className="divide-y divide-ink-100">
                {r.failed.map((f) => (
                  <tr key={f.row}>
                    ...
                    <td className="px-4 py-2 text-ink-600">{f.email ?? "—"}</td>
  ...
  return (
    <form action={run} className="space-y-6">
      ...
      <div>
        ...
        {fileName && (
          <p className="mt-1 text-xs text-ink-500">{fileName} — {rowCount} data row{rowCount === 1 ? "" : "s"} detected.</p>
        )}
      </div>
      ...
      {headers.length > 0 && (
        <>
          ...
          <fieldset className="space-y-3">
            <legend className="text-sm font-semibold text-ink-600">Map columns</legend>
            {RECIPIENT_FIELDS.map((field) => (
              <div key={field} className="flex items-center gap-3">
                <label className="w-32 text-sm text-ink-700" htmlFor={`map-${field}`}>
                  {FIELD_LABELS[field]}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="label" htmlFor="groupId">Add all to group <span className="text-ink-500">(optional)</span></label>
